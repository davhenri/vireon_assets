// mission-core-menu.js - Mission-Core with Mission Selector Support
(async () => {

  // Mission-ID aus Script-Attribut lesen
  let currentScript = document.currentScript;
  if (!currentScript) {
    const scripts = document.querySelectorAll('script[data-default-mission]');
  ...
  let missionId = currentScript.getAttribute('data-default-mission') || 'm1-01';
  const containerId = currentScript.getAttribute('data-container-id');

  console.log('🚀 Mission-Core-Menu startet mit Standard-Mission:', missionId);

  // Hilfsfunktion: Element finden mit Platzhalter-ID
  function getEl(id) {
    return document.getElementById(id + '-CURRENT_MISSION');
  }
  ...
  // DOM-Elemente mit Platzhalter-ID
  ...
  function selectMission(newMissionId) {
    if (newMissionId) missionId = newMissionId;
...
ALLOWED_CALLS = ${JSON.stringify(config.allowedCommands)}
  ...
  async function switchMission(newMissionId) {
    console.log('🔄 Wechsle zur Mission:', newMissionId);
    stopContinue();
    
    // Speichere aktuellen Code
    persistCodeNow();
  ...
  // Make switchMission available globally for potential external use
  window.switchMission = switchMission;
